...
describe('Assets database tests', () => {
  ...
  const entry = { id: 9999, name: 'TestAsset', description: 'TestDescription', material: 'TestMaterial', weight: 1, crown_value: 1, type: AssetType.PRODUCT }
  const entry2 = { id: 9998, name: 'TestAsset2', description: 'TestDescription', material: 'TestMaterial2', weight: 13, crown_value: 2, type: AssetType.PRODUCT }
  ...
  it('should find by filter an entry from the database', () => {
    ...
    expect(db.findValues({ weight: 13 })).toEqual([entry2]);
  ...
  it('should modify an entry from the database', () => {
    db.modifyEntry(9999, { name: 'ModifiedAsset', description: 'ModifiedDescription', material: 'ModifiedMaterial', weight: 2, crown_value: 2 });
    ...
    expect(db.findValues({id: 9999})[0].weight).toBe(2);
